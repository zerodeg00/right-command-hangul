...
설치 스크립트는 macOS의 `문서의 입력 소스로 자동으로 전환` 옵션도 확인합니다.
이 옵션이 켜져 있으면 앱이나 문서가 기억한 입력 소스가 한/영 전환을 다시
덮어쓸 수 있으므로, 설치 결과에 옵션을 끄는 경로를 안내합니다. 사용자 설정을
자동으로 변경하지는 않습니다.
...
3. 전환 뒤 처음 들어오는 일반 키 이벤트를 메모리에 잠깐 보관합니다. 입력 소스
   변경 알림이 먼저 도착했더라도 첫 키는 반드시 이 경로를 거칩니다.
4. macOS가 입력 소스 변경을 완료한 뒤 보관한 이벤트를 원래 순서대로 즉시
   전달합니다. 알림이 오지 않더라도 0.25초 뒤에는 반드시 전달해 키 입력이
   사라지지 않게 합니다.
...
### 메뉴 막대는 한글인데 실제로는 영어가 입력돼요

macOS의 `문서의 입력 소스로 자동으로 전환` 옵션이 켜져 있으면 앱이나 문서별로
기억된 입력 소스가 다시 적용될 수 있습니다. 다음 경로에서 옵션을 꺼 주세요.

> 시스템 설정 → 키보드 → 텍스트 입력 → 편집 → 모든 입력 소스 → 문서의 입력 소스로 자동으로 전환

설치 스크립트도 이 설정을 감지해 켜져 있을 때 경고합니다.
