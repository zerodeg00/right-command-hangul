...
- 전환 직후 입력을 정확히 처리하기 위해 접근성 권한을 요청합니다.
...
처음 설치하면 macOS가 접근성 권한을 요청합니다. 허용한 뒤부터 동작합니다.
...
1. `/usr/bin/hidutil`로 오른쪽 Command와 Right Alt의 HID 코드를 F18에
   매핑합니다.
2. 백그라운드 서비스가 접근성 이벤트 탭에서 F18을 직접 받아 소비하고 Apple의
   Text Input Source API로 한국어와 영문 입력 소스를 전환합니다.
3. 전환 직후의 일반 키 입력은 macOS가 입력 소스 변경을 알릴 때까지 잠시 보관한
   뒤 원래 순서대로 전달합니다.
4. 변경 알림이 누락되더라도 0.25초 안에는 보관한 키를 전달해 입력이 사라지지
   않게 합니다.

폴링하지 않고 macOS 이벤트 큐에서 키 입력을 기다리므로 유휴 상태에서도 첫
입력을 놓치지 않으며, F18이 현재 앱까지 전달되어 경고음이 발생하는 것도
방지합니다. macOS의 입력 소스 단축키 설정을 수정하거나 실제 입력 문자를 읽지 않습니다.
F18은 일반 키보드에 거의 없는 키이므로 다른 키와 충돌할 가능성도 낮습니다.

## 보안과 개인정보

- 관리자 권한을 사용하지 않습니다.
- 접근성 권한은 F18을 소비하고 전환 직후 키 이벤트를 잠시 보관했다가 다시
  전달하는 데에만 사용합니다.
- 입력 모니터링 권한을 요청하지 않습니다.
- 키 입력 내용이나 작성 중인 문자를 기록하거나 저장하지 않습니다.
- 인터넷에 연결하지 않습니다.
...
- macOS 접근성 권한을 허용하지 않으면 전환 기능이 동작하지 않습니다.
